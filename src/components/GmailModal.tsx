            ...
            <tbody>
              {emails.map(email => (
                <tr key={email.id} className="bg-white border-b hover:bg-gray-50">
                  ...
                  <td className="px-3 py-4 font-medium text-gray-900 whitespace-nowrap" title={getSenderName(email.from)}>{truncateText(getSenderName(email.from), 30)}</td>
                  <td className="px-3 py-4 max-w-xs truncate" title={email.subject}>{email.subject}</td>
